...
(defn parse-uri
  ...
  [s]
  (when (string? s)
    (let [[a authority-path query] (re-find uri-pattern s)]
      (when authority-path
        (let [[_ address chain-id function-name] (re-find authority-path-pattern authority-path)]
          (when-not function-name ;; Native token support only
            ;; TODO(jeluard) Add ERC20 support
            (when-not (string/blank? address)
              (merge
                {:address address :chain-id (if chain-id (js/parseInt chain-id) constants/mainnet-id)}
                (parse-query query)))))))))
...
(defn generate-uri
  ...
  (when address
    (when-not function-name ;; Native token support only TODO(jeluard) Add ERC20 support
